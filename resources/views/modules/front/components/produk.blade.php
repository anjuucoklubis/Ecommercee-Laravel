    ...
    <div class="p-6 bg-white dark:bg-gray-800/50 dark:bg-gradient-to-bl from-gray-700/50 via-transparent dark:ring-1 dark:ring-inset dark:ring-white/5 rounded-lg shadow-gray-500/20 dark:shadow-none">
        <div>
            <div class="flex items-center gap-4" style="display: flex;">
                ...
                <h2 class="mt-6 text-xl font-semibold text-gray-900 dark:text-white">PRODUK</h2>
            </div>
            <br>
            <div id="produk-container" class="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 gap-0 lg:gap-8 produk-container">
                @foreach ($GetProdukHomePage as $item)
                <div class="relative bg-white rounded-lg border border-gray-300 shadow-md overflow-hidden flex motion-safe:hover:scale-[1.04] text-decoration-none">
                    <div id="" class="p-4 produk-container">
                        <div class="h-30 w-30 bg-red-50 dark:bg-red-800/20 flex items-center justify-center ">
                            ...
                            <img src="{{ asset('images/products/'.$item->image) }}" alt="wk" class="w-30 h-30" style="width: 300px; height:200px;">
                        </div>
                        <div class="flex items-center gap-4">
                            <h5 class="text-xl font-semibold text-gray-900 dark:text-white" style="font-size:medium">{{ $item->name }}</h5>
                        </div>
                        <div class="flex items-center gap-4">
                            <h5 class="text-xl font-semibold text-gray-900 dark:text-white" style="font-size:small;">Rp {{ $item->price }}.000 </h5>
                            <h5 class="text-xl font-semibold text-gray-900 dark:text-white" style="font-size:small;">-</h5>
                            <h5 class="text-xl font-semibold text-gray-900 dark:text-white" style="font-size:small;">Stok {{ $item->stock }}</h5>
                        </div>
                        @auth
                        <a href="{{ url('detail', $item->id) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full"> Buy Now</a>
                        @else
